  ...
  updateworkshoporderForm() { 
    ...
    this._workshoporderService.getworkshoporderdetails(this.paramObj.serialNumber).subscribe(formdata => {
      this.workshoporderForm = new FormGroup({
        ...
        operatingHours:new FormControl(formdata.operatingHours,[Validators.required]),
        repairDescription:new FormControl(formdata.repairDescription, [Validators.required]),
      });
    })
  }
  setFormdata() {
